...
| `McpTracingInstrumenter` | Built into `quarkus-mcp-server` 1.12.1; auto-creates MCP spans with `requestId`, `toolName`, `session_id` attributes |
...
```
%d{HH:mm:ss} %-5p [%c{2.}] (%t) [traceId=%X{traceId}, requestId=%X{requestId}] %s%e%n
```
