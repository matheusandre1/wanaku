...
Wanaku can run without authentication by setting `wanaku.http.auth=none` (or `WANAKU_ROUTER_HTTP_AUTH=none` via
...
```shell
export WANAKU_ROUTER_HTTP_AUTH=none
...
```shell
java -Dwanaku.router.http-auth=none -jar quarkus-run.jar
```
